...
module.exports = function (files, pattern, opts) {
  ...
  if (_.contains(opts._args, '-Q')) {
    ...
    // Escape dollar sign (bash escape)
    opts.replace = opts.replace.replace(/\$/g, '\\$');

    // Escape backward slashes in the find pattern (bash escape)
  ...
  // Literal option
  if (_.contains(opts._args, '-Q')) {
    prefix = '$re = quotemeta(\'' + perlPattern + '\');';
    perlPattern = '$re';
  }
  ...
  async.mapSeries(files, function (locations, cb) {
    // Execute globs
    glob(locations, {mark: true, dot: true}, function (error, location) {
      if (error) return cb(error);
      location = _.uniq(location);
      // Filter out all directories and .git .hg
      location = _.filter(location, function (file) {
        return file.lastIndexOf('/') !== file.length - 1 && file.indexOf('.git/') !== 0 && file.indexOf('.hg/') !== 0;
      });
    ...
    startProcess(files.join('\n'),
      ...
      function (error, out, err) {
        startProcess(out, 'xargs', [perlCmd, '-pi', '-e', perlRPattern],
